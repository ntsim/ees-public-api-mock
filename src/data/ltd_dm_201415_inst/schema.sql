...
CREATE TABLE locations(id UINTEGER PRIMARY KEY DEFAULT(nextval('locations_seq')), geographic_level VARCHAR NOT NULL, country_code VARCHAR, country_name VARCHAR, region_code VARCHAR, region_name VARCHAR, old_la_code VARCHAR, new_la_code VARCHAR, la_name VARCHAR, school_urn VARCHAR, school_name VARCHAR);
